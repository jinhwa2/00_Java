...
public class ClientEx {
	...
	private static final int SERVER_PORT = 12802; // 서버가 열어놓은 포트번호 서버 = 클라이언트 포트 같아야함
	...
	public static void main(String[] args) {
		...
		Thread serverThread = new Thread(() -> {
			// 서로에서 방문자한테 메세지를 보내는 값으로 처음에는 아무 대화도 없기 때문에 null을 줌
			...
			while (isRunning) {
				...
				try {
					//만약에 서버랑 클라이언트 연결이 끊어졌따면 1초마다 다시 재접속 시도
					Thread.sleep(1000);
