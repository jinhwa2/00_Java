...
/*
  ...
  interface : 클래스 간의 접점을 만드는 용도 ->내가 이걸 지정했고 이걸 변경하지마의 뜻
  필드에 작성하는 모든 변수가
  ...
  Q : 그렇다면 인터페이스는 왜 public static final 과 public abstract 가 붙는겁니까?
  A : 인터페이스 자바에서 다른 클래스에서 구현할 메서드의 명세를 정의하는 역할을 함
  	  인터페이스에 작성되는 필드와 메서드는 다른 클래스에서 구현될 것을 기대하는 것
 
 	내가 주는 필드 값은 강제야 변경할 생각 하지 말기>포인트
 */
public interface 기계 {
//필드
